...
function update(sourcePath, dependentPath) {
    const source = JSON.parse(fs_1.readFileSync(`${sourcePath}/package.json`, 'utf8'));
    const dependent = JSON.parse(fs_1.readFileSync(`${dependentPath}/package.json`, 'utf8'));
    let isBump = false;
    if (dependent['dependencies'][source.name] !== source.version) {
        dependent['dependencies'][source.name] = source.version;
        fs_1.writeFileSync(`${dependentPath}/package.json`, JSON.stringify(dependent, null, 2), 'utf8');
        isBump = true;
    }
    return {
